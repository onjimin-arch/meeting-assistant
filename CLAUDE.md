...
5. **Android 빌드 버전 임의 변경 금지**: Kotlin / AGP / Gradle 버전은 아래 고정값만 사용
   - Kotlin: `2.1.0` / AGP: `8.5.2` / Gradle: `8.10.2` (Flutter 3.29 공식 템플릿 검증 조합)
   - 변경이 필요하면 반드시 해당 버전의 실제 릴리즈 여부를 공식 릴리즈 페이지에서 확인 후 진행
   - 과거 사례: Kotlin `2.3.20`(미존재), AGP `9.0.1`(Flutter 미지원) → 빌드 실패 반복
