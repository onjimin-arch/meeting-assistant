...
이번에 만들어지는 APK의 동작 상태:

- ✅ 6개 화면 전부 동작 (홈/녹음/처리/회의록/채팅/설정)
- ✅ 화면 전환, 설정 저장 (Notion 토큰 등), 빠른 옵션 선택
- ✅ **STT (음성 → 텍스트) 실제 동작** — 폰에 탑재된 Google/Apple 음성인식 엔진 사용
  - 녹음 버튼 → 마이크 권한 허용 → 말하기 → 중단하면 실제로 인식된 한국어 텍스트가 transcript에 들어감
  - 회의처럼 긴 발화도 OK: 60초마다 시스템이 끊는 것을 자동 재시작으로 보완
- ⚠️ **회의록 생성·Notion 저장은 아직 mock 응답**
  - STT 결과는 진짜지만, 그걸 받아 회의록을 만드는 Gemma 부분은 미통합
  - 화면에 표시되는 회의록은 하드코딩된 예시
  - 실제 AI 회의록/Notion 저장은 후속 작업(5~6단계, CLAUDE.md 참고)에서 진행 예정

### STT 동작 조건

- 인터넷 연결 권장 (오프라인 한국어 STT 지원 기종이 제한적)
- Google 앱이 설치되어 있어야 함 (대부분 안드로이드에 기본 설치)
- 첫 실행 시 **마이크 권한** 팝업이 뜸 → 허용
- 일부 안드로이드에서 "음성 인식 서비스가 없습니다" 에러가 나면:
  Play 스토어에서 "Speech Services by Google" 또는 "Google" 앱 업데이트
...
**Q. APK 크기가 얼마나 되나요?**
A. 이번 빌드 APK는 약 20~30MB. STT는 폰 내장 엔진을 쓰므로 추가 모델 다운로드 없음. Gemma(LLM)만 통합 후 첫 실행 시 약 1.5GB 별도 다운로드 예정. APK 자체는 계속 30~50MB 유지.
